#!/usr/bin/env node
/**
 * 独立识图脚本 — 调用硅基流动 Vision 模型 (nex-agi/Nex-N2-Pro)
 *
 * 用法:
 ...
 *   VISION_API_KEY / VISION_BASE_URL / VISION_MODEL 环境变量 或 同目录 .env 文件
 */
...
async function main() {
  const missing = [];
  if (!API_KEY) missing.push("VISION_API_KEY");
  if (!BASE_URL) missing.push("VISION_BASE_URL");
  if (!MODEL) missing.push("VISION_MODEL");
  if (missing.length) {
    console.error(`请设置以下环境变量或在 .env 文件中配置: ${missing.join(", ")}`);
    if (missing.includes("VISION_API_KEY"))
      console.error("获取 Key: https://cloud.siliconflow.cn/");
    process.exit(1);
  }
